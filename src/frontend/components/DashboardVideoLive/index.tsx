...
const DashboardVideoLiveButton = styled(BtnWithLink)`
...
export const DashboardVideoLive = ({ video }: DashboardVideoLiveProps) => {
  ...
  const [interval, setInterval] = useState<Nullable<number>>(null);
  ...
  if (video.live_state === liveState.STARTING && !interval) {
    setInterval(window.setInterval(() => pollForVideo(), 1000 * 15));
  }

  if (interval && video.live_state !== liveState.STARTING) {
